...
// Load Profile Model
const Profile = require('../../models/UniversityProfile');
// Load User Model
const User = require('../../models/University');
...
router.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      // Add to exp array
      profile.course.unshift(newCourse);
...
// @desc    Delete courses from profile
// @access  Private
router.delete(
  '/courses/:course_id',
  passport.authenticate('jwt', { session: false }),
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        // Get remove index
        const removeIndex = profile.course
          .map(item => item.id)
          .indexOf(req.params.course_id);

        // Splice out of array
        profile.course.splice(removeIndex, 1);
